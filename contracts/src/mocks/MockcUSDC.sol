...
/**
 * @title MockcUSDC
 * @notice Mock version of cUSDC for local testing without Zama coprocessor
 * @dev Uses MockFHE library to simulate encrypted balances
 */
contract MockcUSDC {
    ...
    string public constant name = "Confidential USDC (Mock)";
    ...
    function mint(address to, uint256 amount) external onlyWrapper {
        ...
        euint64 encryptedAmount = MockFHE.asEuint64(uint64(amount));

        if (MockFHE.isInitialized(_encryptedBalances[to])) {
            _encryptedBalances[to] = MockFHE.add(_encryptedBalances[to], encryptedAmount);
        } else {
    ...
    function burn(address from, uint256 amount) external onlyWrapper {
        ...
        euint64 encryptedAmount = MockFHE.asEuint64(uint64(amount));
        _encryptedBalances[from] = MockFHE.sub(_encryptedBalances[from], encryptedAmount);
    ...
    function transferEncryptedFrom(
        ...
        _encryptedBalances[from] = MockFHE.sub(_encryptedBalances[from], amountHandle);

        // Add to recipient
        if (MockFHE.isInitialized(_encryptedBalances[to])) {
            _encryptedBalances[to] = MockFHE.add(_encryptedBalances[to], amountHandle);
        } else {
    ...
    function hasBalance(address account) external view returns (bool) {
        return MockFHE.isInitialized(_encryptedBalances[account]);
    }

    /**
     * @notice Get decrypted balance (MOCK ONLY - for testing)
     */
    function balanceOfDecrypted(address account) external view returns (uint64) {
        return MockFHE.decrypt(_encryptedBalances[account]);
    }
}
